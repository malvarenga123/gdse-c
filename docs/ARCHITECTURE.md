...
- `--full-rainbow` reproduces the Rainbow Filter scheme closely but not exactly. Successive supplied outputs exposed both improvements and regressions; the latest outputs confirm 12 base, 9 GDX1, and 8 GDX2 color-only differences. Crafted-result and boss-chest rules are scoped by expansion localization namespaces, while generic tier paths remain excluded so ordinary Rare, Epic, and Legendary pools do not become Monster Infrequents. `docs/AUDIT.md` records the full accounting.
